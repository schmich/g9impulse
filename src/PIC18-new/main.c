...
void introLoop()
{
    ...
    if (scene1() || scene2() || scene3() || scene4() || scene5() || scene6())
        return;
}
...
void drawarray (Sprite sprite[])
{
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        {        
            ...
            load_s_lines(sprite[i].image.height);
    ...
    else if ( (unit.visible >= 211) && (unit.visible <= 222))
    {
        ...
        else
        {
            ...
            if (unit.a % updateRate == 0)
            {
                ...
                {
                    unit.image.address += 160 * unit.image.height - 3 * unit.image.width;
                }
                else
                {
                    unit.image.address += unit.image.width;
    ...
    for (i = 0; i < 12; i++)
    {
        if (enemy[i].visible && !isExplosion(&enemy[i]))
        {
            if(((enemy[i].image.width + enemy[i].x) > px) &&
                (enemy[i].x < (pw + px)) &&
                (enemy[i].y < (ph + py)) &&
               ((enemy[i].image.height + enemy[i].y) > py) )
            {
        ...
        for (i = 0; i < 12; i++)
        {
                ...
                (missile[j].visible != VISIBLE_Dead))
            {
                if( ((enemy[i].image.width + enemy[i].x) > missile[j].x) &&
                   ...
                   ((enemy[i].image.height + enemy[i].y) > missile[j].y) )
                {
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        if ((enemy[i].visible == 0))
        {
            ...
            else if ( (type >= 150) && (type <= 157))
            {
                ...
                char yt = enemy[from].image.height;
                enemy[i].image.address = 0x000D97BB;
                enemy[i].image.height = 7;
...
Entity* createPlayer(Point where)
{
    Entity* player = (Entity*)alloc(sizeof(Entity));
    player->position = where;

    player->image = (Image*)alloc(sizeof(Image));
    player->image->address = 0x000D7A20;
    player->image->width = 16;
    player->image->height = 28;
...
void handlePlayerInput(Entity* player)
{
    ...
    {
        player->position.x += 2;
        if (player->position.x + player->image->width > 160) // HACK should be SCREEN_WIDTH
            player->position.x = 160 - player->image->width;
    }
    ...
    {
        player->position.y += 4;
        if (player->position.y + player->image->height > (240 + 5)) // HACK should be SCREEN_HEIGHT
            player->position.y = (240 + 5) - player->image->height;
    }
}
